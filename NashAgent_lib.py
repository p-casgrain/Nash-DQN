...
class NashNN():
    ...
    def __init__(self, non_invar_dim, output_dim, n_players, max_steps, terminal_cost, num_moms=5):
        ...
        if torch.cuda.is_available():
            self.action_net = PermInvariantQNN(
                in_invar_dim=self.num_players - 1, non_invar_dim=self.non_invar_dim, out_dim=output_dim, num_moments=num_moms).cuda()
            self.value_net = PermInvariantQNN(
                in_invar_dim=self.num_players - 1, non_invar_dim=self.non_invar_dim, out_dim=1).cuda()
        else:
            self.action_net = PermInvariantQNN(
                in_invar_dim=self.num_players - 1, non_invar_dim=self.non_invar_dim, out_dim=output_dim, num_moments=num_moms)
            self.value_net = PermInvariantQNN(
                in_invar_dim=self.num_players - 1, non_invar_dim=self.non_invar_dim, out_dim=1)
    ...
    def predict_action(self, states):
        ...
        if torch.cuda.is_available():
            action_list = self.action_net.forward(
                invar_input=inv_states.cuda(), non_invar_input=expanded_states.cuda())
        else:
            action_list = self.action_net.forward(
                invar_input=inv_states, non_invar_input=expanded_states)

        NFV_list = []
        for i in range(0, len(states)):
            NFV_list.append(NashFittedValues(
                action_list[i*self.num_players:(i+1)*self.num_players, :]))
    ...
    def compute_value_Loss(self, state_tuples):
        ...
        isLastState = np.repeat(
            np.array([s.t <= 0 for s in next_state_list]).astype(int), self.num_players)

        # isLastState = torch.cat(self.num_players*[torch.tensor(
        #     [s.t <= 0 for s in next_state_list], dtype=torch.float32)])
        ...
        nextstate_val = self.predict_value(
            next_state_list).detach().view(-1).data.cpu()
    ...
    def compute_action_Loss(self, state_tuples):
        ...
        curVal = self.predict_value(cur_state_list).detach(
        ).view(-1).cpu()                    # Nash Value of Current state
        nextVal = self.predict_value(next_state_list).detach(
        ).view(-1).cpu().data.numpy()    # Nash Value of Next state
        ...
        term_list = \
            np.array(list(map(
                obj_map,
                expanded_next_states[:, 1],
                expanded_next_states[:, 2],
                self.terminal_cost*np.ones(len(expanded_next_states)))))
