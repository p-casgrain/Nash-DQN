...
class MarketSimulator(object):
    ...
    def __init__(self, param_dict_in, store_hist=False):
        ...
        self.sigma0 = np.sqrt(param_dict['initial_price_var'])
        self.sigma_Q0 = np.sqrt(param_dict['init_inv_var'])
    ...
    def reset(self):
        ...
        """
        self.Q=np.random.normal(0, self.sigma_Q0, self.N)
        self.S=np.float32(10 + np.random.normal(0, self.sigma0))
        ...
        if self.store_hist:
            init_state, _, _=self.get_state()
            self.state_hist=[init_state]
            self.reward_hist=[]
    ...
    def step(self, nu):
        ...
        if not self.done:
            ...
            self.dF=self.mu(self.t, self.S) * self.dt + \
                            self.sigma * self.dW[int(round(self.t/self.dt))]
            ...
            self.dI=self.I * (np.exp(-self.tmp_decay*self.dt) - 1) + \
                self.tmp_scale * self.impact_scale(np.sum(nu))
            self.I += self.dI
            self.dS=self.dF + self.dI + (self.perm_imp * np.mean(nu))
            self.S += self.dS

            # Compute Action Reward
            self.last_reward=self.r(self.t, self.Q, self.S, self.dS, nu)
            self.total_reward += self.last_reward

        cur_state, _, _=self.get_state()

        if self.store_hist:
            self.state_hist.append(cur_state)
            self.reward_hist.append(self.last_reward)
