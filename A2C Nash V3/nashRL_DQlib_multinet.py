...
class NashNN():
    ...
    def predict_action(self, states):
        expanded_states = self.expand_list(states)
        action_list = self.action_net.forward(torch.tensor(expanded_states).float())
    ...
    def predict_value(self, states):
        expanded_states = self.expand_list(states)
        return self.value_net.forward(torch.tensor(expanded_states).float())
    ...
    def compute_action_Loss(self, state_tuples):
        ...
        return torch.sum((torch.tensor(np.multiply(np.ones(len(curVal))-flag, nextVal) + np.multiply(flag, term_list) + reward_list.view(-1)).float()
                        ...
                        + penalty*torch.var(c2_list.view(-1,self.num_players),1).view(-1,1).repeat(1,2).view(-1)
                        + penalty*torch.var(c3_list.view(-1,self.num_players),1).view(-1,1).repeat(1,2).view(-1))
