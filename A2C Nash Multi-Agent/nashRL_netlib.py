...
if __name__ == '__main__':
    net = PermInvariantQNN(in_invar_dim = 2, non_invar_dim = 2,
                 out_dim = 1)
    a = torch.tensor([[3,20],[20,3]]).float()
    b = torch.tensor([[1,3],[1,8]]).float()
    print(net.forward(invar_input = a,non_invar_input = b))
    
    a = torch.tensor([[20,3],[4,30]]).float()
    b = torch.tensor([[1,2],[3,4]]).float()
    print(net.forward(invar_input = a,non_invar_input = b))
    
    a = torch.tensor([[20,3],[4,30]]).float()
    b = torch.tensor([[2,1],[3,4]]).float()
    print(net.forward(invar_input = a,non_invar_input = b))